...
class Security extends CI_Controller { 
    ...
    public function index() {
        if($this->session->userdata('security_login') != 1) redirect(base_url(). 'login', 'refresh');
    ...
    function dashboard() {
        ...
        $page_data['page_title'] =  get_phrase('Security Dashboard');
    ...
    public function update_status() {
        // $this->load->model('Request_model'); // Load the model

        $request_id = $this->input->post('request_id');

        if ($request_id) {
            $status = $this->security_model->update_status($request_id);
            echo json_encode(['status' => $status ? 'success' : 'failed']);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Invalid request ID']);
        }
    }
    public function update_status_2() {
        // $this->load->model('Request_model'); // Load the model

        $request_id = $this->input->post('request_id');

        if ($request_id) {
            $status = $this->security_model->update_status_2($request_id);
            echo json_encode(['status' => $status ? 'success' : 'failed']);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Invalid request ID']);
        }
    ...
    function manage_request ($param1 = null, $param2 = null, $param3 = null){
        ...
        if($param1 == ''){
            ...
            $match = array('running_year' => $running_year, 'status =' => 'approved');
            $page_data['status'] = 'active';
            // $this->db->order_by('exam_date', 'desc');
            // $page_data['online_exams'] = $this->db->where($match)->get('online_exam')->result_array();
    ...
    function view_request($request_id){
        ...
        $page_data['page_title'] = get_phrase('request');
        $this->load->view('backend/index', $page_data);
    }
    ...
    public function update_days_overstayed() {

    $request_id = $this->input->post('request_id');
    $days_overstayed = $this->input->post('days_overstayed');

    // Load model
    $this->load->model('Security_model');

    // Update days overstayed in the database
    $this->Security_model->updateDaysOverstayed($request_id, $days_overstayed);
