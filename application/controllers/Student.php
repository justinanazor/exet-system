...
class Student extends CI_Controller { 
    ...
    function create_request(){
        $page_data['page_name'] = 'add_request';
        $page_data['page_title'] =  get_phrase('Create Request');
        $this->load->view('backend/index', $page_data);
    }
    ...
    function manage_request ($param1 = null, $param2 = null, $param3 = null){
        ...
        $student_info = $this->db->get_where('student', array('student_id' => $this->session->userdata('login_user_id')))->row()->hostel;

        $admin_hostel = $this->db->get_where('hostel', array('type' => $student_info))->row()->email;

        if($param1 == ''){
           
            $match = array('status !='  => 'expired','running_year' => $running_year, 'student_id' => $this->session->userdata('login_user_id'));
        ...
        if($param1 == 'declined'){
             ...
             $match = array('status !=' => 'declined', 'running_year' => $running_year);
             $page_data['status'] = 'declined';
            //  $this->db->order_by('exam_date', 'desc');
             $page_data['requests'] = $this->db->where($match)->get('requests')->result_array();
         }
         ...
         if($param1 == 'approved'){
            ...
            $match = array('status' => 'approved', 'running_year' => $running_year);
        ...
        if($param1 == 'create'){
            ...
            // if ($this->input->post('class_id') > 0 && $this->input->post('section_id') > 0 && $this->input->post('subject_id') > 0) {
                ...
                $email_to = $admin_hostel;
                $email_subject = 'Request Notification';
                $email_desc = 'Dear Hostel Admin, kindly check your account for a request.';

                $this->trigger_mail($email_to, $email_subject, $email_desc);
            // }
            // else {
                $this->session->set_flashdata('success_message', get_phrase('Request sent to Hostel admin successfully'));
   ...
   function trigger_mail($to, $subject, $message, $cc = NULL)
	...
	$CI->email->from('', 'Veritas Exeat Management System'); // change it to yours
